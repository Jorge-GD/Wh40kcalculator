...
import { CalculationResult, HitProbabilities, TotalResults } from '../models/calculation-result.model';
...
export class CalculationService {
   ...
   */
  private getRawProb(targetRoll: number, diceSides: number = this.DICE_SIDES): number {
   ...
   */
  getHitProbabilities(baseTarget: number, critThreshold: number, rerollType: string): HitProbabilities {
    let effectiveBaseTarget = Math.max(2, Math.min(this.DICE_SIDES, baseTarget));
    let effectiveCritThreshold = Math.max(2, Math.min(this.DICE_SIDES, critThreshold));
    effectiveCritThreshold = Math.max(effectiveBaseTarget, effectiveCritThreshold);

    let pCritRaw = this.getRawProb(effectiveCritThreshold);
    let pNormalRaw = Math.max(0, this.getRawProb(effectiveBaseTarget) - pCritRaw);
    ...
    if (rerollType === 'ones') {
      ...
      if (1 < effectiveBaseTarget) { // Rerolling a 1 can only help if 1 is a fail
    ...
    if (totalSuccess > 1) { // Normalize if probabilities exceed 1 due to rerolls
  ...
  private applyFNP(damage: number, defenderProfile: DefenderProfile, isMortal: boolean): number {
    const generalFnpActive = defenderProfile.feelNoPain?.active || false;
    const generalFnpTarget = generalFnpActive ? (defenderProfile.feelNoPain?.value || 0) : 0;
    let probPassFnp = 0;
    ...
    if (isMortal) {
      ...
      const fnpVsMortalsTarget = fnpVsMortalsActive ? (defenderProfile.fnpVsMortals?.value || 0) : 0;
      if (fnpVsMortalsActive && fnpVsMortalsTarget >=2 && fnpVsMortalsTarget <= this.DICE_SIDES) {
        fnpTargetToUse = fnpVsMortalsTarget;
      } else if (generalFnpActive && generalFnpTarget >=2 && generalFnpTarget <= this.DICE_SIDES) {
        fnpTargetToUse = generalFnpTarget;
      }
    } else { // Normal damage
      if (generalFnpActive && generalFnpTarget >=2 && generalFnpTarget <= this.DICE_SIDES) {
        fnpTargetToUse = generalFnpTarget;
      }
    }
  ...
  private getProbFailFNP(defenderProfile: DefenderProfile, isMortal: boolean): number {
    const generalFnpActive = defenderProfile.feelNoPain?.active || false;
    const generalFnpTarget = generalFnpActive ? (defenderProfile.feelNoPain?.value || 0) : 0;
    let probPassFnp = 0;
    let fnpTargetToUse = 0;
  ...
  private calculateDamageComponents(attackerProfile: AttackerProfile, defenderProfile: DefenderProfile): ProfileDamagePotential {
    ...
    let habilidadObjetivo = data.skill || 7;
    ...
    let habilidadModificada = Math.max(2, Math.min(this.DICE_SIDES + 1, habilidadObjetivo - modImpactoNeto));

    // Calcular umbral crítico de impacto
    let umbralCriticoImpacto = 6;
    if (data.critHitMod?.active) umbralCriticoImpacto = data.critHitMod?.value || 5;
    umbralCriticoImpacto = Math.max(2, Math.min(this.DICE_SIDES, umbralCriticoImpacto));
    ...
    if (data.torrent) {
      ...
      const hitProbs = this.getHitProbabilities(habilidadModificada, umbralCriticoImpacto, data.hitRerollType || 'none');
    ...
    let heridaModificada = Math.max(2, Math.min(this.DICE_SIDES + 1, heridaObjetivoBase - modHerirNeto));

    let umbralCriticoHerida = this.DICE_SIDES;
    if (data.anti?.active) umbralCriticoHerida = Math.min(umbralCriticoHerida, data.anti?.value || this.DICE_SIDES);
    umbralCriticoHerida = Math.max(2, Math.min(this.DICE_SIDES, umbralCriticoHerida));
    umbralCriticoHerida = Math.max(heridaModificada, umbralCriticoHerida);

    const actualWoundRerollType = data.twinLinked?.active ? 'all' : (data.woundRerollType || 'none');
    const woundProbs = this.getHitProbabilities(heridaModificada, umbralCriticoHerida, actualWoundRerollType);
    let eHeridasCriticasWoundRoll = impactosQueHieren * woundProbs.probCrit;
    let eHeridasNormalesWoundRoll = impactosQueHieren * woundProbs.probNormal;
    const totalWoundsInflictedPreDevastating = eAutoHeridasPorLetales + eHeridasNormalesWoundRoll + eHeridasCriticasWoundRoll;
    ...
    if (reduceDamageHalved) dañoNormalPorInstancia = Math.ceil(dañoNormalPorInstancia / 2);
    if (flatReduction > 0) dañoNormalPorInstancia = Math.max(1, dañoNormalPorInstancia - flatReduction);

    // Heridas devastadoras (pre-FNP)
    let eHeridasMortalesDevastadoras_PreFNP = 0;
    let heridasQueRequierenSalvacion = eAutoHeridasPorLetales + eHeridasNormalesWoundRoll; // Start with auto-wounds and normal wounds from roll
    ...
    if (data.devastatingWounds?.active) {
      ...
      if (reduceDamageHalved) dañoPorDevastadora = Math.ceil(dañoPorDevastadora / 2);
      if (flatReduction > 0) dañoPorDevastadora = Math.max(1, dañoPorDevastadora - flatReduction);
      
      eHeridasMortalesDevastadoras_PreFNP = eHeridasCriticasWoundRoll * dañoPorDevastadora;
    } else {
    ...
    if (data.extraMortals?.active && data.extraMortals.on && data.extraMortals.amount) {
      let cantidadMortalesHabilidad = this.interpretDiceValue(data.extraMortals.amount || "0");
      ...
      if (reduceDamageHalved) cantidadMortalesHabilidad = Math.ceil(cantidadMortalesHabilidad / 2);
      if (flatReduction > 0) cantidadMortalesHabilidad = Math.max(1, cantidadMortalesHabilidad - flatReduction);

      if (!data.torrent) { // Assuming trigger on hit roll
        const probTriggerMortal = this.getRawProb(data.extraMortals.on);
        eHeridasMortalesPorHabilidad_PreFNP = numAtaquesBase * probTriggerMortal * cantidadMortalesHabilidad;
      }
    }
    ...
    let salvacionBase = defenderProfile.saveSv || 7;
    ...
    salvacionFinalObjetivo = Math.max(2, Math.min(this.DICE_SIDES + 1, salvacionFinalObjetivo));
    const eSalvacionesFalladas = heridasQueRequierenSalvacion * (1 - this.getRawProb(salvacionFinalObjetivo));

    const eDañoNormalTotal_PreFNP = eSalvacionesFalladas * dañoNormalPorInstancia;

    // Aplicar Feel No Pain
    const normalDamagePostFNP = this.applyFNP(eDañoNormalTotal_PreFNP, defenderProfile, false);
    const mortalWoundsDevastatingPostFNP = this.applyFNP(eHeridasMortalesDevastadoras_PreFNP, defenderProfile, true);
   ...
   */
  calculateTotalDamage(attackerProfiles: AttackerProfile[], defenderProfiles: DefenderProfile[]): TotalResults {
    const aggregatedResultsMap = new Map<string, CalculationResult>(); // Key: attackerId-defenderId
    let grandTotalDamage = 0;

    const defenderStates = new Map<string, { 
        id: string;
        name: string;
        hp: number; 
        models: number; 
    ...
    const sortedDefenderStatesList = Array.from(defenderStates.values()).sort((a, b) => a.saveOrder - b.saveOrder);

    for (const attackerProfile of attackerProfiles) {
        let attackerAbilityMortalsPoolPreFNP = 0;

        const firstDefenderForBlastState = sortedDefenderStatesList.find(ds => ds.models > 0) || sortedDefenderStatesList[0];
        if (firstDefenderForBlastState) {
            const potential = this.calculateDamageComponents(attackerProfile, firstDefenderForBlastState.profile);
        ...
        for (const defState of sortedDefenderStatesList) {
            ...
            const components = this.calculateDamageComponents(attackerProfile, defState.profile);
            let nonSpillDamageThisPair = components.normalDamagePostFNP + components.mortalWoundsDevastatingPostFNP;
            
            let damageAppliedToThisDefender = Math.min(nonSpillDamageThisPair, defState.hp);
            defState.hp -= damageAppliedToThisDefender;
            grandTotalDamage += damageAppliedToThisDefender;
            
            const oldModels = defState.models;
            defState.models = defState.hp > 0 ? Math.ceil(defState.hp / defState.woundsPerModel) : 0;
            ...
            if (damageAppliedToThisDefender > 0 && nonSpillDamageThisPair > 0) {
                 pairResult.mortalWoundsByDevastating += (components.mortalWoundsDevastatingPostFNP / nonSpillDamageThisPair) * damageAppliedToThisDefender;
            }
        ...
        for (const defState of sortedDefenderStatesList) {
            ...
            const probFailFNPForMortals = this.getProbFailFNP(defState.profile, true);
            let maxPreFNPMortalsToKill = Infinity;
            if (probFailFNPForMortals > 0) { // Avoid division by zero if FNP always passes
                 maxPreFNPMortalsToKill = defState.hp / probFailFNPForMortals;
            } else if (defState.hp > 0) { // FNP always passes, can\'t kill with mortals unless 0 HP
                 continue; // Cannot deal damage if FNP always passes and HP > 0
            }


            let mortalsToAttemptOnThisDefenderPreFNP = Math.min(remainingAbilityMortalsPreFNP, maxPreFNPMortalsToKill);
            mortalsToAttemptOnThisDefenderPreFNP = Math.max(0, mortalsToAttemptOnThisDefenderPreFNP); // Ensure non-negative


            let mortalsPostFNPForThisDefender = this.applyFNP(mortalsToAttemptOnThisDefenderPreFNP, defState.profile, true);
            let damageAppliedFromAbilityMW = Math.min(mortalsPostFNPForThisDefender, defState.hp);
            ...
            remainingAbilityMortalsPreFNP -= mortalsToAttemptOnThisDefenderPreFNP; // Reduce pool by what was attempted
             if(remainingAbilityMortalsPreFNP < 0) remainingAbilityMortalsPreFNP = 0;
            ...
            const resultKey = `${attackerProfile.id}-${defState.id}`;
            let pairResult = aggregatedResultsMap.get(resultKey);
            if (!pairResult) { // Should exist if attacker could target this defender at all
                 pairResult = { /* ... initialize ... */ profileName: attackerProfile.name, defenderProfileName: defState.name, finalDamage:0, modelsKilled:0, mortalWoundsByAbility:0, expectedHits: 0, expectedWounds: 0, failedSaves: 0, mortalWoundsByDevastating: 0 };
                 aggregatedResultsMap.set(resultKey, pairResult);
            }
